...
// A simple hook to get user info from sessionStorage.
...
export default function StudentDashboard() {
  const { user: authUser, isUserLoading: isAuthLoading } = useUser(); // Firebase auth user
  const { user: sessionUser, isLoading: isSessionLoading } = useSimpleUser(); // sessionStorage user
  ...
  const isLoading = isAuthLoading || isSessionLoading || studentLoading || stocksLoading;

  if (isLoading) {
    return <div className="flex min-h-screen items-center justify-center bg-background">Loading...</div>;
  }
  
  if (!sessionUser || sessionUser.type !== "student" || !student) {
