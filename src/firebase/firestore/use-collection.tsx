...
export function useCollection<T = any>(
  ...
  useEffect(() => {
    // If the query is not ready, set loading to false and don't subscribe.
    // The loading state will be true on initial render and then change based on query status.
    if (!stableQuery) {
      setData(null);
      setIsLoading(false); 
      setError(null);
    ...
    // When a query is provided, we are actively loading.
    setIsLoading(true);
    setError(null);
    ...
    const unsubscribe = onSnapshot(
      ...
      (error: FirestoreError) => {
        ...
        setData(null)
        setIsLoading(false) // Loading finished, but with an error
