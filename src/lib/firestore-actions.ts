...
export const redeemReward = async (firestore: Firestore, studentId: string, rewardId: string) => {
    ...
    try {
        ...
        // We can't know which operation failed inside transaction, but we can provide some context
         const permissionError = new FirestorePermissionError({
            path: `students/${studentId} or rewards/${rewardId}`,
            operation: 'update',
            requestResourceData: { note: "Transaction failed, likely due to a permission issue on student or reward documents."}
        });
        errorEmitter.emit('permission-error', permissionError);
